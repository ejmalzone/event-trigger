...
public class NuiWsLogSource implements EventSource {

	private static final Logger log = LoggerFactory.getLogger(NuiWsLogSource.class);
	...
	private static final URI defaultUri = URI.create("ws://127.0.0.1:32805/ws");
	...
	private final class ActWsClientInternal extends WebSocketClient {
		...
		public void onOpen(ServerHandshake serverHandshake) {
			...
			eventConsumer.accept(new NuiWsConnectedEvent());
		}

		@Override
		public void onMessage(String s) {
			log.trace("WS Message: {}", s);
			eventConsumer.accept(new NuiWsRawMsg(s));
		}
		...
		public void onClose(int i, String s, boolean b) {
			...
			eventConsumer.accept(new NuiWsDisconnectedEvent());
		}
		...
		public void onError(Exception e) {
			...
			eventConsumer.accept(new NuiWsReconnectRequest());
		}
...
//			send("{\"call\":\"subscribe\",\"events\":[\"OnlineStatusChanged\"]}");
	...
	public NuiWsLogSource(EventMaster master, StateStore stateStore, PersistenceProvider pers, PrimaryLogSource pls) {
		String settingBase = "nextui-ws.";
		this.uriSetting = new WsURISetting(pers, settingBase + "uri", defaultUri);
		this.allowBadCert = new BooleanSetting(pers, settingBase + "allow-bad-certs", false);
		this.eventConsumer = master::pushEvent;
		// TODO: it doesn't have TTS...
		this.allowTts = new BooleanSetting(pers, settingBase + "enable-tts", true);
		this.pls = pls;
...
//
	...
	public void forceReconnect(EventContext context, DebugCommand event) {
		if (event.getCommand().equals("reconnect_nui_ws")) {
			log.info("Reconnect requested");
			context.accept(new NuiWsReconnectRequest());
		}
	}
...
//	}
	...
	@LiveOnly
	@HandleEvents
	public void requestReconnect(EventContext context, NuiWsReconnectRequest event) {
	...
	@LiveOnly
	@HandleEvents
	public void reconnectActWs(EventContext context, NuiWsDisconnectedEvent event) {
	...
	static {
		try {
			String[] cbtProps = {"CurrentWorldID", "WorldID", "WorldName", "BNpcID", "BNpcNameID", "PartyType", "ID", "OwnerID", "type", "Job", "Level", "Name", "CurrentHP", "MaxHP", "CurrentMP", "MaxMP", "PosX", "PosY", "PosZ", "Heading", "TargetID"};
			allCbtRequest = mapper.writeValueAsString(Map.ofEntries(Map.entry("call", "getCombatants"), Map.entry("rseq", "allCombatants"), Map.entry("props", cbtProps)));
			specificCbtRequestTemplate = mapper.writeValueAsString(Map.ofEntries(Map.entry("call", "getCombatants"), Map.entry("rseq", "specificCombatants"), Map.entry("ids", List.of(123456)), Map.entry("props", cbtProps)));
		}
		catch (JsonProcessingException e) {
