...
public class NuiWsHandlers {

	private static final Logger log = LoggerFactory.getLogger(NuiWsHandlers.class);
	private static final ObjectMapper mapper = new ObjectMapper();
	private final EventMaster master;
	private final XivState state;
	private final PullTracker pulls;

	public NuiWsHandlers(EventMaster master, XivState state, PullTracker pulls) {
		this.master = master;
		this.state = state;
		this.pulls = pulls;
	}
	...
	@LiveOnly
	@HandleEvents
	public void startCombatantsLoop(EventContext context, NuiWsConnectedEvent connected) {
	...
	public void actWsRawToJson(EventContext context, NuiWsRawMsg rawMsg) {
		JsonNode jsonNode;
...
//				}
//				// Just fake the type
//				NuiWsJsonMsg nuiWsJsonMsg = new NuiWsJsonMsg("combatants", rseqObj, jsonNode);
//				context.accept(nuiWsJsonMsg);
//			}
//			return;
//		}
		JsonNode typeNode = jsonNode.path("event");
		String type;
		...
		NuiWsJsonMsg nuiWsJsonMsg = new NuiWsJsonMsg(type, null, jsonNode);
		context.accept(nuiWsJsonMsg);
	}

	@HandleEvents(order = -100)
	public static void actWsLogLine(EventContext context, NuiWsJsonMsg jsonMsg) {
	...
	@HandleEvents(order = -100)
	public static void actWsOnlineStatusChanged(EventContext context, NuiWsJsonMsg jsonMsg) {
	...
	@HandleEvents(order = -100)
	public static void actWsPlayerChange(EventContext context, NuiWsJsonMsg jsonMsg) {
	...
	@HandleEvents(order = -100)
	public static void actWsZoneChange(EventContext context, NuiWsJsonMsg jsonMsg) {
	...
	@HandleEvents(order = -100)
	public static void actWsPartyChange(EventContext context, NuiWsJsonMsg jsonMsg) {
	...
	@HandleEvents(order = -100)
	public static void inCombatChange(EventContext context, NuiWsJsonMsg jsonMsg) {
...
//	}
	...
	@HandleEvents(order = -100)
	public void actWsCombatants(EventContext context, NuiWsJsonMsg jsonMsg) {
